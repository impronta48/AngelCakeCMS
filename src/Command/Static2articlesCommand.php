  ...
  private function creaArticles($risult, $path)
  {
    ...
    foreach ($risult as $r) {
      ...
      if ($ex == 0) {
        ...
        $article->keywords = isset($r['dati']['keywords']) ? $r['dati']['keywords'] : '';
        $article->url_canonical = isset($r['dati']['canonical']) ? $r['dati']['canonical'] : '';
        $article->body = $r['dati']['body'];
        $article->modified = $r['dati']['date'];
        $article->published = true;
        $article->tag_string = isset($r['dati']['canonical']) ? $r['dati']['canonical'] : '';
