    ...
    <table cellpadding="0" cellspacing="0" class="table table-responsive">
        ...
        <?php foreach (${$tableAlias} as $user) : ?>
            <tr>
                ...
                <td class="actions">
                    <?= $this->Html->link('', ['action' => 'view', $user->id], ['class' => 'btn btn-default glyphicon glyphicon-eye-open']) ?>
                    <?= $this->Html->link('', ['action' => 'changePassword', $user->id],['class'=>'btn btn-default glyphicon fa fa-key ']) ?>
                    <?= $this->Html->link('', ['action' => 'edit', $user->id],['class' =>  'btn btn-default glyphicon glyphicon-pencil']) ?>
                    <?= $this->Form->postLink('', ['action' => 'delete', $user->id], ['confirm' => __d('CakeDC/Users', 'Are you sure you want to delete # {0}?', $user->id), 'class' => 'btn btn-default glyphicon glyphicon-trash']) ?>
                </td>
            </tr>
